...
class DeterministicCaptionChunkTests(unittest.TestCase):
    ...
    def test_preserves_decimal_number_as_one_caption_token(self) -> None:
        chunks = self.build_text_chunks(
            "StatPearls puts it at about 87.8 percent fast-twitch fibers, so the rep should feel quick and precise, not crushed.",
            max_words=3,
        )
        self.assertNotIn("at about 87", chunks)
        self.assertNotIn("8 percent", chunks)
        self.assertTrue(any(chunk.startswith("87.8 percent") for chunk in chunks), chunks)

    def test_preserves_decimal_number_before_sentence_period(self) -> None:
