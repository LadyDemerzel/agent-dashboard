...
import { getShortFormSoundLibraryDir } from "@/lib/short-form-sound-design-settings";

export const dynamic = "force-dynamic";
export const runtime = "nodejs";

function resolveSafePath(parts: string[]) {
  const baseDir = path.resolve(getShortFormSoundLibraryDir());
  const resolvedPath = path.resolve(baseDir, ...parts);
  if (resolvedPath !== baseDir && !resolvedPath.startsWith(`${baseDir}${path.sep}`)) {
    throw new Error("Invalid sound library path");
  }
  return resolvedPath;
}
...
export async function GET(
  ...
  try {
    ...
    if (!fs.existsSync(resolvedPath)) {
      return NextResponse.json({ success: false, error: "Saved sound-library file not found" }, { status: 404 });
    }
    ...
    if (rangeHeader && baseHeaders.get("Content-Type")?.startsWith("audio/")) {
    ...
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : "Failed to read saved sound-library file" },
      { status: 400 }
    );
  }
}
