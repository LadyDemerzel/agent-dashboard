...
function resolveSafePath(parts: string[]) {
  const baseDir = path.resolve(getShortFormVoiceLibraryDir());
  const resolvedPath = path.resolve(baseDir, ...parts);
  if (resolvedPath !== baseDir && !resolvedPath.startsWith(`${baseDir}${path.sep}`)) {
    throw new Error("Invalid voice library path");
  }
  return resolvedPath;
}
...
export async function GET(
  ...
  try {
    ...
    if (!fs.existsSync(resolvedPath)) {
      return NextResponse.json({ success: false, error: "Saved voice sample not found" }, { status: 404 });
    }
    ...
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : "Failed to read saved voice sample" },
      { status: 400 }
    );
  }
}
