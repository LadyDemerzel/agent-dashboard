...
export function ResearchDetailClient({ file, content, statusLog }: ResearchDetailClientProps) {
  ...
  }>(`/api/research/${file.id}/versions`, jsonFetcher, realtimeSWRConfig);
  ...
  const diffKey = viewMode === "changes" && currentVersion && compareVersion
    ? `/api/research/${file.id}/diff?from=${compareVersion}&to=${currentVersion}`
    : null;
  ...
  const {
    ...
    isLoading: loading,
    mutate: fetchThreads,
  } = useSWR<{ threads: FeedbackThreadType[] }>(`/api/research/${file.id}/feedback/threads`, jsonFetcher, {
    ...realtimeSWRConfig,
