...
export function InlineFeedbackThread({
  ...
  const handleResolve = async () => {
    if (onResolveThread) await onResolveThread(thread.id);
  };

  const handleReopen = async () => {
    if (onReopenThread) await onReopenThread(thread.id);
  };
          ...
          <div className="space-y-3">
            {thread.comments.map((comment) => (
              <div key={comment.id} className="flex gap-2">
                <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 ${comment.author === "user" ? "bg-blue-500/20" : "bg-purple-500/20"}`}>
