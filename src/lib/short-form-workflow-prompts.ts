...
const DEFAULT_SHORT_FORM_WORKFLOW_PROMPTS: ShortFormWorkflowPrompts = {
  ...
  videoGenerate: [
    "Generate the final short-form video using the Agent Dashboard repo-owned xml-scene-video renderer.",
    "Topic: {{topic}}",
    ...
    "Use the repo-bundled scripts/xml-scene-video/generate_video.py workflow rather than rebuilding the ffmpeg/TTS/music pipeline ad hoc.",
    "Recommended command shape: uv run --with pillow python3 scripts/xml-scene-video/generate_video.py --xml {{scriptPath}} --images-dir {{sceneImagesDir}} --output {{finalVideoPath}} --work-dir {{videoWorkDir}} --tts-engine qwen --existing-voice {{xmlNarrationPath}} --existing-alignment {{xmlAlignmentPath}} --no-music --force",
